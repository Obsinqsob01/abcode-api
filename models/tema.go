...
// GetAllTemaByCourseId retrieves all Tema matches certain condition. Returns empty list if
// no records exist
func GetAllTemaByCourseId(query map[string]string, fields []string, sortby []string, order []string,
	...
	var l []Tema
	qs = qs.OrderBy(sortFields...).RelatedSel()
	if _, err = qs.Limit(limit, offset).Filter("course_id", id).All(&l, fields...); err == nil {
		if len(fields) == 0 {
			for _, v := range l {
				ml = append(ml, v)
			}
